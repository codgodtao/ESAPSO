...
def print_current_training_stats(error_pan_model, error_ms_model, error_g_model, global_step, learning_rate,
                                 time_elapsed):
...
def main(argv):
    ...
    model.train()
    dataset = DataSet(FLAGES.pan_size, FLAGES.ms_size, FLAGES.train_data_path, FLAGES.batch_size,
                      FLAGES.stride)
    DataGenerator = dataset.data_generator

    dataset_valid = DataSet(FLAGES.pan_size, FLAGES.ms_size, FLAGES.valid_data_path, FLAGES.batch_size,
                            FLAGES.stride, 'valid')
    ...
    with tf.Session() as sess:
        ...
        for training_itr in range(FLAGES.iters):
            ...
            for i in range(2):
                ...
                _, error_ms_model = sess.run([model.train_spectrum_discrim, model.spectrum_loss],
                                             feed_dict={model.pan_img: pan_batch, model.ms_img: ms_batch})
            ...
            print_current_training_stats(error_pan_model, error_ms_model, error_g_model, global_step, learning_rate,
                                         time.time() - t1)
