...
class PanGan(object):
    ...
    def __init__(self, pan_size, ms_size, batch_size, num_spectrum, ratio, init_lr=0.001, lr_decay_rate=0.99,
                 lr_decay_step=1000, is_training=True):
    ...
    def build_model(self, pan_size, ms_size, batch_size, num_spectrum, is_training):
        ...
        if is_training:
            with tf.name_scope('input'):
                ...
                self.ms_img = tf.placeholder(dtype=tf.float32, shape=(batch_size, ms_size, ms_size, num_spectrum),
                                             name='ms_placeholder')
            ...
            with tf.name_scope('PanSharpening'):
                ...
                self.PanSharpening_img_ = tf.image.resize_images(images=self.PanSharpening_img, size=[ms_size, ms_size],
                                                                 method=tf.image.ResizeMethod.BILINEAR)  # 使用双线性插值降低分辨率
                self.PanSharpening_img_hp = self.high_pass_1(self.PanSharpening_img)  # 分别获取梯度信息
            ...
            with tf.name_scope('input'):
                ...
                self.pan_img = tf.placeholder(dtype=tf.float32, shape=(batch_size, None, None, 1),
                                              name='pan_placeholder')
                self.ms_img = tf.placeholder(dtype=tf.float32, shape=(batch_size, None, None, num_spectrum),
                                             name='ms_placeholder')
            self.PanSharpening_img = self.PanSharpening_model_dense(self.pan_img, self.ms_img)
            # self.PanSharpening_img_blur=self.blur(self.PanSharpening_img,7)
    ...
    def train(self):
        ...
        with tf.name_scope('train_step'):
            ...
            self.train_spectrum_discrim = tf.train.RMSPropOptimizer(self.learning_rate).minimize(self.spectrum_loss,
                                                                                                 var_list=d_spectrum_vars)
        ...
        with tf.variable_scope('Pan_model'):
            if self.is_training:
                with tf.name_scope('upscale'):
                    ...
                    ms_img = tf.image.resize_images(ms_img, [256, 256], method=2)
    ...
    def resnet_PanSharpening_model_dense(self, pan_img, ms_img):
        def residual_block(inputs, output_channel, stride, scope):
            with tf.variable_scope(scope):
                ...
                net2 = Op.conv2(tf.concat([inputs, net1], -1), 3, output_channel, stride, use_bias=False,
                                scope='conv_2')
    ...
    def spatial_discriminator(self, img_hp, reuse=False):
        with tf.variable_scope('spatial_discriminator', reuse=reuse):
            ...
            with tf.variable_scope('layer_6'):
                weights = tf.get_variable("w_6", [3, 3, 256, 512],
                                          initializer=tf.truncated_normal_initializer(stddev=1e-3))
                bias = tf.get_variable("b_6", [512], initializer=tf.constant_initializer(0.0))
            ...
            with tf.variable_scope('line_7'):
                weights = tf.get_variable("w_7", [4, 4, 512, 1],
                                          initializer=tf.truncated_normal_initializer(stddev=1e-3))
                bias = tf.get_variable("b_7", [1], initializer=tf.constant_initializer(0.0))
    ...
    def spectrum_discriminator(self, img, reuse=False):
        with tf.variable_scope('spectrum_discriminator', reuse=reuse):
            ...
            with tf.variable_scope('layer5_spectrum'):
                ...
                # conv4_spectrum = tf.reshape(conv4_spectrum, [self.batch_size, 1 * 1 * 128])
            with tf.variable_scope('layer6_spectrum'):
                weights = tf.get_variable("w6_spectrum", [3, 3, 256, 512],
                                          initializer=tf.truncated_normal_initializer(stddev=1e-3))
                bias = tf.get_variable("b6_spectrum", [512], initializer=tf.constant_initializer(0.0))
                conv6_spectrum = tf.contrib.layers.batch_norm(
                    tf.nn.conv2d(conv5_spectrum, weights, strides=[1, 2, 2, 1], padding='SAME') + bias, decay=0.9,
                    updates_collections=None, epsilon=1e-5, scale=True)
                conv6_spectrum = self.lrelu(conv6_spectrum)
                # conv4_spectrum = tf.reshape(conv4_spectrum, [self.batch_size, 1 * 1 * 128])

            with tf.variable_scope('line7_spectrum'):
                weights = tf.get_variable("w7_spectrum", [4, 4, 512, 1],
                                          initializer=tf.truncated_normal_initializer(stddev=1e-3))
                bias = tf.get_variable("b7_spectrum", [1], initializer=tf.constant_initializer(0.0))
                conv7_spectrum = tf.contrib.layers.batch_norm(
                    tf.nn.conv2d(conv6_spectrum, weights, strides=[1, 1, 1, 1], padding='VALID') + bias, decay=0.9,
                    updates_collections=None, epsilon=1e-5, scale=True)
                conv7_spectrum = self.lrelu(conv7_spectrum)
                conv7_spectrum = tf.reshape(conv7_spectrum, [self.batch_size, 1])
                # line5_spectrum = tf.matmul(conv4_spectrum, weights) + bias
                # conv3_vi= tf.contrib.layers.batch_norm(conv3_vi, decay=0.9, updates_collections=None, epsilon=1e-5, scale=True)
        return conv7_spectrum
